 ...
 * @extends BaseScheme
 */
class SameDayScheme extends BaseTokenScheme {
     ...
     */
    get sameDaySecret() {
        return _.get(this.sameDayOptions, 'secret', null)
    }

    /**
     * The jwt public
     *
     * @attribute sameDayPublic
     * @type {String|Null}
     * @readOnly
     */
    get sameDayPublic() {
        return _.get(this.sameDayOptions, 'public', null)
    }
    ...
    _signToken(payload, options) {
        options = _.size(options) && _.isPlainObject(options) ? options : _.omit(this.sameDayOptions, ['secret', 'public'])
        return signToken(payload, this.sameDaySecret, options)
    }
     ...
     */
    _verifyToken(token) {
        const options = _.omit(this.sameDayOptions, ['secret', 'public'])
        const secretOrPublicKey = this.sameDayPublic !== null ? this.sameDayPublic : this.sameDaySecret
    ...
    async generate(user, jwtPayload, jwtOptions) {
         ...
         * jwt secret
         */
        if (!this.sameDaySecret) {
            throw GE.RuntimeException.incompleteConfig(['secret'], 'config/auth.js', 'sameday')
        }
         ...
         * @type {Object}
         */
        const payload = { uid: userId, sub: userId }
    ...
    async check() {
         ...
         */
        try {
            this.sameDayPayload = await this._verifyToken(token)
        } catch ({ name, message }) {
        ...
        this.user = await this._serializerInstance.findById(this.sameDayPayload.sub || this.sameDayPayload.uid)
